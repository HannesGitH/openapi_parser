...
pub struct EndpointParser<'a> {
    pub params_parser:
        &'a dyn Fn(&'a Vec<ObjectOrReference<Parameter>>) -> Result<Vec<Param<'a>>, Error>,
    pub request_parser:
        &'a dyn Fn(Option<&'a ObjectOrReference<RequestBody>>) -> Result<IAST<'a>, Error>,
    pub responses_parser: &'a dyn Fn(
